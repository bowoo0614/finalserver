...
export default function FeedList() {
  ...
  const fetchFeeds = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get<Feed[]>('http://localhost:4000/feeds');
  ...
  const fetchComments = async (feedId: number) => {
    try {
      const response = await axios.get<Comment[]>(`http://localhost:4000/feeds/${feedId}/comments`);
      setComments(prev => ({
        ...prev,
  ...
  const handleCommentSubmit = async (feedId: number) => {
    ...
    try {
      ...
      await axios.post(`http://localhost:4000/feeds/${feedId}/comments`, {
        content,
      });
